...
def three_proposals_ready(email, content):
...
def final_reminder_project(email,content):
	#14 after 3 ready
...
def cancel_warning_project(email,content):
...
def request_for_service_12hrs(email,content):
	subject = 'Job request from ' + content['customer_name']
	send(email, content, subject, 'request_for_service_12hrs')

def send_a_message_to_project(email,content):
